...
public class FloatMatrix
{
    ...
    private static void transposeSquare(float[] data, int offset, int n1, int n2)
    {
        ...
        int cacheBurstBlockSize = Util.round2down(ctx.getCacheBurst() / 4),   // Cache burst in floats
            cacheBlockSize = Util.sqrt4down(ctx.getCacheL1Size() / 4),        // Transpose block size b that fits in processor L1 cache
            cacheTreshold = Util.round2down(ctx.getCacheL2Size() / 4);        // Size of matrix that fits in L2 cache
    ...
    private static void permuteWideToTall(float[] data, int offset, int n1, int n2)
    {
        assert (n2 == 2 * n1);
    ...
    private static void permuteTallToWide(float[] data, int offset, int n1, int n2)
    {
        assert (n1 == 2 * n2);
