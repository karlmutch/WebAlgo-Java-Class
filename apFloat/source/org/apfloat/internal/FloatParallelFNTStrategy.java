    ...
    implements ParallelNTTStrategy
{
    ...
    private class MultiplyRunnable
        implements Runnable
    {
        public MultiplyRunnable(ArrayAccess arrayAccess, int startRow, int rows, int columns, float w, float scaleFactor)
        {
            this.arrayAccess = arrayAccess;
        ...
        public void run()
        {
            ...
            float tmp = modPow(this.w, (float) this.startRow);

            for (int i = 0; i < this.rows; i++)
            {
                float tmp2 = this.scaleFactor;
    ...
    protected void multiplyElements(final ArrayAccess arrayAccess, final int startRow, final int rows, final int columns, final float w, final float scaleFactor)
    ...
    protected void transformRows(final int length, final int count, final boolean isInverse, final ArrayAccess arrayAccess, final float[] wTable, final int[] permutationTable)
        throws ApfloatRuntimeException
    {
        ParallelRunnable parallelRunnable = new ParallelRunnable()
        {
